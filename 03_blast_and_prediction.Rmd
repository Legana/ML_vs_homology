...
methods_auprc %>% 
  ...
  ggplot(aes(x = Method, y = AUPRC, fill = Method)) +
  geom_col(width = 0.7) +
  facet_wrap(~factor(Organism, levels = c("Homo sapiens", "Pan troglodytes","Mus musculus" , "Rattus norvegicus" , "Bos taurus", "Sus scrofa", "Canis lupus familiaris", "Rhinolophus ferrumequinum")), ncol = 2, scales = "free") +
  theme_classic() +
  ...
  labs(fill = "") +
  scale_fill_manual(breaks = c("BLAST1", "BLAST2", "Classification"),
                       values = c("forestgreen","violetred", "grey1")) 


ggsave("figures/auprcs_mammals.png", width = 6, height = 5)
```
...
For classification models, generally a probability value of 0.5 or higher is considered to be a positive value. This means any protein predicted at a probability of 0.5 or higher is considered to be an AMP. The bitscores for the BLAST results range between 0 and `r max(blast2results$bitscore)` According to [Pearson 2013, pp. 4-5](https://doi.org/10.1002/0471250953.bi0301s42): "For average length proteins, a bit score of 50 is almost always significant. A bit score of 40 is only significant (E() < 0.001) in searches of protein databases with fewer than 7000 entries". Therefore, a bitscore threshold of 50 or higher was used to categorise a protein as an AMP.
...
ggplot(blast1and2andpred_roc, aes(x = Recall, y = Precision) ) +
  ...
  geom_text(aes(label = label), data = figure_text, check_overlap = TRUE, vjust = "top", hjust = "right", size = 4, fontface = "bold") +
...
ggsave("figures/mammals_prcurve.png", width = 10, height = 9)
...
ggsave(plot = blast1and2andpred_roc_plot_theme_black, "figures/mammals_prcurve_black.png", width = 10, height = 9)
