...
**Figure 7.1:** **A** Histogram of pairwise distance between each faceted organism and 648 out of the 788 other organisms present in the AMP dataset
...
```{r, echo = FALSE}
organism_strict_selection <- c("Mus musculus","Homo sapiens","Bos taurus","Oryctolagus_cuniculus","Ornithorhynchus anatinus","Gallus gallus", "Drosophila melanogaster","Bombyx mori","Arabidopsis thaliana")
```
...
**Figure 7.3:** The contributions of different species to the taxonomic representation score of the selected organisms. Only the two species with the largest contributions are shown. The contribution of the remaining species were amalgamated into the "other" category. 

```{r, echo = FALSE}
ggsave("figures/species_contribution_to_score.png", width = 9, height = 5)
```


```{r, echo = FALSE}
ggplot(filter(amps_distance_grouped, Target == "Ornithorhynchus anatinus"), aes(x = Target)) + 
  geom_col(aes(y = dscore, fill = Organism2), width = 0.3) + 
  labs(y = "Taxonomic representation score", x = "", fill = "") +
  theme_classic() +
  theme(legend.position = "bottom",
        axis.text.y = element_text(face = "italic")) +
  guides(fill = guide_legend(label.theme = element_text(face = "italic", size = 9)))
...
**Figure 7.6:** **A** Scatter and line plot of the summed inverse pairwise distance and the AUPRC for each AMP finding method for AMPs in different organisms. The size of points depends on the number of AMPs in the organism, represented by the AMP_count.
