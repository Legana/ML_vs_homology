...
theplot <- ggplot(auprc_and_distance_metric_long, aes(x = Inverse_distance_sum, y = AUPRC)) +
...
**Figure 4.3:** **A** Scatter and line plot of the summed inverse pairwise distance and the AUPRC for each AMP finding method for AMPs in different organisms. The AMP_count reflects the number of AMPs known in each organism.


```{r, fig.height=6, echo = FALSE}
auprc_no_primates_scatter <- ggplot(filter(auprc_and_distance_metric_long, !Organism %in% c("Homo_sapiens", "Pan_troglodytes")), aes(x = Inverse_distance_sum, y = AUPRC, colour = Method)) +
  geom_point() +
...
auprc_no_primates_line <- ggplot(filter(auprc_and_distance_metric_long, !Organism %in% c("Homo_sapiens", "Pan_troglodytes")), aes(x = Inverse_distance_sum, y = AUPRC, colour = Method)) +
  geom_line() +
  labs(x = "The sum of the inverse pairwise distance", colour = "") +
  theme_classic() +
  theme(legend.position = "bottom")

auprc_no_primates_scatter / auprc_no_primates_line + plot_annotation(tag_levels = "A")

ggsave("figures/auprc_vs_distance_scatter_linear.png", width = 7, height = 7)

```

**Figure 4.3:** **A** Scatterplot and **B** line plot of the summed inverse pairwise distance and the AUPRC for each AMP finding method, excluding the primates *Homo sapiens* and *Pan troglodytes*
...
how many AMPs present for each *chosen* organism in new training DB. use range of taxonomic distances. Stretch out x axis.
10 borderline, over 20 is good. 


what to do about unreviewed proteins?

represent number of AMPs for each organism in plot
scatterplot, size of point depends on number of AMPs
how many AMPs if reviewed OR unreviewed, AND in AMP databases
